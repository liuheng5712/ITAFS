function [] = Attack(index, round, Toselect, TrTeRatio)

% this is the old version, where the poisoning is semi_greedy
...
max_expense = 0.5; %maximal appended points: ceil(max_expense*n)
...
% % keep track of AverageConsistency of SOP poisoning for 3 objectives
% SOPConsistency_mrmr = [1];
% SOPConsistency_jmi = [1];
% SOPConsistency_mifs = [1];
% % To control data Sampling based on NN, obtain its consistency w/ the clean data
% NAveConsistency_mrmr = [1];
% NAveConsistency_jmi = [1];
% NAveConsistency_mifs = [1];
...
for i = 2:f
    [F0_critical, F1_critical, Y_critical, expense] = JonPoisingGenr(train_data(:, best_feature), train_data(:, sorted_features(i)), y, max_expense);
    if expense < ceil(max_expense*n) & expense > 0 %the poisoning is successful with allowed expense
        Expense(i) = expense;
        
%         %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%         %use nearest neighbor to find the data samples which can carry malicious features first
%         malicious_data = train_data;
%         authentic_row = train_data(:, best_feature);
%         fake_row = train_data(:, sorted_features(i));
%         for j = n+1:n+expense    %find the carrier for each row of three malicious vectors
%             which_sample = 0;
%             minimum = 1000000;
%             for q = 1:n %iterate original data samples
%                 if minimum > (F0_critical(j) - authentic_row(q))^2 + (F1_critical(j) - fake_row(q))^2 + (Y_critical(j) - y(q))^2
%                     minimum = (F0_critical(j) - authentic_row(q))^2 + (F1_critical(j) - fake_row(q))^2 + (Y_critical(j) - y(q))^2;
%                     which_sample = q;
%                 end
%             end
%             malicious_data = [malicious_data; train_data(which_sample,:)];
%         end
        %Or we can use random sampling to get the carrier for malicious features, BUT Rememeber to change the mrmr and mifs SOP poisoning codes ！！！
